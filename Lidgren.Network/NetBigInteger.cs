...
namespace Lidgren.Network
{
    ...
    internal class NetBigInteger
    {
        ...
        public override bool Equals(object? obj)
        {
            ...
            if (bigInteger.m_sign != m_sign || bigInteger.m_magnitude.Length != m_magnitude.Length)
            {
                return false;
            }

            for (var magnitudeIndex = 0; magnitudeIndex < m_magnitude.Length; magnitudeIndex++)
            {
                if (bigInteger.m_magnitude[magnitudeIndex] != m_magnitude[magnitudeIndex])
                {
                    return false;
                }
            }
